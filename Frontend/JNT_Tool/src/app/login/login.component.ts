      ...
      try {
        ...
        if (error) {
          ...
          const { data: userData, error: fetchError } = await this.supabase
            .from('usertable')
            .select('id, tenantName,firstName, userId, department') // Add 'tenantName' to the select query
            .eq('email', email)
            .single();
          ...
          if (fetchError) {
            ...
            this.snackBar.open('Login Successful', 'OK', { duration: 3000, horizontalPosition: 'right' });
